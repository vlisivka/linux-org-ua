...
$managetemplatesplver = 'YaBB 2.1 $Revision: 1.3 $';
 ...
 <div class="bordercolor" style="padding: 0px; width: 99%; margin-left: 0px; margin-right: auto;">
   <table width="100%" cellspacing="1" cellpadding="4" style="table-layout: fixed;">
	...
	<tr valign="middle">
		...
		<input type="submit" value="$admin_txt{'10'} $templatefile" />
		</form>
		</td>
	</tr>
	<tr valign="middle">
		...
		<noscript><input type="submit" value="$admin_txt{'32'}" /></noscript>
		</form>
		</div>
		</td>
	</tr>
...
sub ModifyStyle {
	...
	fopen(CSS, "$csstype") or &admin_fatal_error("$admin_txt{'23'} $csstype");
 ...
 <div class="bordercolor" style="padding: 0px; width: 99%; margin-left: 0px; margin-right: auto;">
   <table width="100%" cellspacing="1" cellpadding="4">
	...
	<tr>
		...
		<input type="submit" name="wysiwyg" id="wysiwyg" value="wysiwyg" />
	...
	<tr valign="middle">
		<td align="center" class="catbg">
		<input type="submit" value="$admin_txt{'10'} $cssfile" />
		</td>
	</tr>
	</form>
	<tr>
		...
		<div class="small" style="float: left; width: 25%;">Forum:<br />
		...
		<noscript><input type="submit" value="$admin_txt{'32'}" style="width: 90%;" /></noscript>
		</div>
		</form>
		<form action="$adminurl?action=modstyle" name="aselcss" method="post" style="display: inline;">
		<div class="small" style="float: left; width: 25%;">Admin Center:<br />
...
sub ModifyCSS {
	...
	fopen(CSS, "$forumstylesdir/$cssfile") or &admin_fatal_error("$admin_txt{'23'} $forumstylesdir/$cssfile");
	@thecss = <CSS>;
	fclose(CSS);
	...
	$stylestr =~ s/\s{2,}/ /g;
	my ($selstyl, $selhidden, $postsstyle, $seperatorstyle, $bodycontainerstyle, $bodystyle, $containerstyle, $titlestyle, $titlestyle_a, $categorystyle, $categorystyle_a, $window1style, $window2style, $inputstyle, $textareastyle, $selectstyle, $quotestyle, $codestyle, $editbgstyle, $highlightstyle);
	my ($gen_fontsize);
	...
	if ($stylestr =~ /input/) {
		...
		$selstyl .= qq~<option value='$inputstyle'>$templ_txt{'34'}</option>\n~;
	}
...
<table width="100%" cellspacing="1" cellpadding="4">
	<tr>
		...
		<input type="submit" name="source" id="source" value=" source " />
		</form>
		</td>
	</tr>
</table>
...
<table width="100%" cellspacing="1" cellpadding="4">
	<tr>
		<td align="left" class="windowbg2">
		<div style="float: left; width: 25%; padding: 3px;"><b>$templ_txt{'1'}</b></div>
	...
	<tr>
		...
		<div style="float: left; text-align: center; margin-left: 4px; margin-right: 6px; vertical-align: middle;">
		...
		<div style="float: left;">
			...
			<div style="float: left; height: 68px; width: 86px; overflow: auto; border: 1px; border-style: outset; margin-left: 8px;">
			~;
	for ($z = 0; $z < 256; $z += 51) {
		$c3 = sprintf("%02x", $z);
		for ($y = 0; $y < 256; $y += 51) {
			...
			for ($x = 0; $x < 256; $x += 51) {
				...
				$yymain .= qq~<span style="background-color: $selcolor; width: 11px; height: 11px;"><img src="$imagesdir/palette.gif" border="0" hspace="0" vspace="0" alt="" style="height: 11px; width: 11px; margin: 0px;" onclick="previewColor('$selcolor')" /></span>~;
			}
			$yymain .= qq~<br />~;
		}
	}
...
<div id="messagesa" class="message"><a href="#">$templ_txt{'66'}</a><br /><br /></div>
...
<input type="button" value="$templ_txt{'34b'}" />
</td>
</tr>
<tr>
<td id="window1" class="windowbg" align="left" valign="top">
...
~;
	...
	<tr valign="middle">
		...
		<div style="float: left; width: 25%; padding: 3px;"><b>$templ_txt{'12'}</b></div>
		<div style="float: left; width: 74%;">
			<input type="text" name="savecssas" size="30" maxlength="30" />
			<input type="submit" value="$templ_txt{'13'}" onclick="document.allstyles.button.value = '2';" />
		</div>
		</td>
	</tr>
</table>
...
sub ModifyCSS2 {
	...
	if ($FORM{'button'} eq "2") {
		...
		if ($style_name eq "default") { &admin_fatal_error($templ_txt{'17'}); }
		if ($style_name !~ m^\A[0-9a-zA-Z_\.\#\%\-\:\+\?\$\&\~\.\,\@/]+\Z^ || $style_name eq "") { &admin_fatal_error($templ_txt{'16'}); }
	...
	if ($FORM{'button'} eq "3") {
		...
		if ($style_name eq "default.css") { &admin_fatal_error($templ_txt{'17'}); }
...
sub ModifySkin {
	...
	else { $displaysel = qq~ checked="checked"~; }
	...
	$tempnewstitle = qq~<b>$maintxt{'102'}:</b> ~;
	$tempnews      = qq~This is your News messages (if news enabled)~;
	...
	$tempmenu = qq~$img{'home'}$menusep$img{'help'}$menusep$img{'search'}$menusep$img{'memberlist'}$menusep$img{'profile'}$menusep$img{'notification'}$menusep$img{'logout'}~;
	$tempmenu =~ s~img src\=\"$imagesdir\/(.+?)\"~&TmpImgLoc($1, $tempimages, $tempimagesdir)~eisg;

	$tempuname = qq~$maintxt{'247'} ${$uid.$username}{'realname'}, ~;
	$tempuim   = qq~$load_txt{'152'} <a name="ims">0 $load_txt{'153'}</a>.~;
	...
	$fulltemplate =~ s/<yabb menu>/$tempmenu/g;
	...
	if ($selectedsection eq "vboard") {
		...
		$messagetempl = &MessageTempl($viewboard, $tempimages, $tempimagesdir);
		$fulltemplate =~ s/<yabb main>/$messagetempl/g;
	} elsif ($selectedsection eq "vdisplay") {
		$displaytempl = &DisplayTempl($viewboard, $tempimages, $tempimagesdir);
...
<table width="100%" cellspacing="1" cellpadding="4">
	<tr valign="middle">
		<td align="left" class="windowbg2">
		<div style="float: left; width: 30%; padding: 3px;"><b>$templ_txt{'10'}</b></div>
		<form action="$adminurl?action=modskin2" name="alltemplates" method="post" style="display: inline;">
		<div style="float: left; width: 69%;">
				<input type="hidden" name="button" value="0" />
				<select name="templateset" id="templateset" size="1" onchange="document.alltemplates.button.value = '1'; submit();">
				$templatesel
				</select>
~;
	unless ($akttemplate eq "Forum default") {
		$yymain .= qq~<input type="submit" value="$templ_txt{'14'}" onclick="document.alltemplates.button.value = '3'; return confirm('$templ_txt{'15'} $thistemplate?')" />~;
	}
	$yymain .= qq~
		</div>
		</form>
	...
	<tr>
		<td align="left" class="windowbg2">
		<div style="float: left; width: 30%; padding: 3px;"><b>$templ_txt{'11'}</b><br /><span class="small">$templ_txt{'7'}</span></div>
...
sub ModifySkin2 {
	...
	if ($FORM{'button'} eq "2") {
		...
		if ($template_name eq "default") { &admin_fatal_error($templ_txt{'17'}); }
		if ($template_name !~ m^\A[0-9a-zA-Z_\ \.\#\%\-\:\+\?\$\&\~\.\,\@/]+\Z^ || $template_name eq "") { &admin_fatal_error($templ_txt{'16'}); }
		($template_css, $dummy, $dummy) = split(/\./, $FORM{'cssfile'});
		$template_images = $FORM{'imgfolder'};
		($template_head,    $dummy) = split(/\./, $FORM{'headfile'});
		($template_board,   $dummy) = split(/\//, $FORM{'boardfile'});
		($template_message, $dummy) = split(/\//, $FORM{'messagefile'});
		($template_display, $dummy) = split(/\//, $FORM{'displayfile'});
	...
	if ($FORM{'button'} eq "3") {
		...
		if ($template_name eq "default")       { &admin_fatal_error($templ_txt{'17'}); }
		if ($template_name eq "Forum default") { &admin_fatal_error($templ_txt{'17'}); }
...
sub BoardTempl {
	...
	my $latestmemberlink = qq~$boardindex_txt{'201'} <a href="#"><b>${$uid.$username}{'realname'}</b></a>.<br />~;
	my $tempims          = qq~$boardindex_txt{'795'} <a href="#"><b>2</b></a> $boardindex_txt{'796'} $boardindex_imtxt{'24'} <a href="#"><b>2</b></a> $boardindex_imtxt{'26'}.~;
	my $tempforumurl     = $mbname;
	my $tempnew          = qq~<img src="$_[1]/off.gif" alt="" border="0" />~;
	my $tempcurboard     = qq~Template Test Board~;
	my $tempboardanchor  = qq~Template_Test_Board~;
	my $tempbddescr      = qq~This is the Board Description~;
	my $tempshowmods     = qq~$boardindex_txt{'63'}: moderator names<br />$boardindex_txt{'63a'}: moderator groups~;
	my $templastposttme  = &timeformat($date);
	my $templastpostlink = qq~<a href="#">$img{'lastpost'}</a> $templastposttme~;
	my $templastposter   = qq~<a href="#">${$uid.$username}{'realname'}</a>~;
	my $tmplasttopiclink = qq~<a href="#">Link to last Topic</a>~;
	$tempcatlink = qq~<img src="$_[1]/cat_collapse.gif" alt="" border="0" /> <a href="#">Category Name link</a>~;
	...
	for ($i = 1; $i < 3; $i++) {
		...
		$templateblock =~ s/<yabb boardurl>/$tempcurboard/g;
		$templateblock =~ s/<yabb boardpic>//g;
	...
	$boardindex_template =~ s/<yabb navigation>/$tempforumurl/g;
	...
	my $templasttopiclink = qq~$boardindex_txt{'236'} <a href="#"><b>Link to last Topic</b></a>~;
...
sub MessageTempl {
	...
	my $tempcatnm        = qq~Test category~;
	my $tempboardnm      = qq~Test board~;
	my $tempmodslink     = qq~($messageindex_txt{'298'}: Mod names)~;
	my $tempbdescrip     = qq~This is the board description.~;
	...
	my $tempmsublink     = qq~This is the subject link.~;
	my $tempmname        = qq~${$uid.$username}{'realname'}~;
	my $templastpostlink = qq~<img src="$_[1]/lastpost.gif" style="vertical-align: middle;" alt="" border="0" /> Link to last post~;
	my $templastposter   = qq~$tempmname~;
	my $tempyabbicons    = qq~
	...
	<img src="$_[1]/locked.gif" alt="" /> $messageindex_txt{'456'}
	~;
	...
	$messageindex_template =~ s/<yabb forumjump>//g;
...
sub DisplayTempl {
	...
	if ($MenuType == 0) {
		...
		$yimimg = qq~$menusep<img src="$html_root/Buttons/$language/yim.png" alt="" border="0" />~;
		$aimimg = qq~$menusep<img src="$html_root/Buttons/$language/aim.png" alt="" border="0" />~;
		$msnimg = qq~$menusep<img src="$html_root/Buttons/$language/msn.png" alt="" border="0" />~;
	}
	my $template_home        = qq~<span class="nav">$mbname</span>~;
	my $tempcatnm            = qq~Test category~;
	my $tempboardnm          = qq~Test board~;
	my $tempmodslink         = qq~($display_txt{'298'}: Mods)~;
	my $template_prev        = qq~$display_txt{'768'}~;
	my $template_next        = qq~$display_txt{'767'}~;
	my $temppageindextgl     = qq~<img src="$_[1]/xx.gif" style="vertical-align: middle;" alt="" border="0" />~;
	...
	my $template_threadimage = qq~<img src="$imagesdir/thread.gif" align="middle" alt="" />~;
	my $threadurl            = qq~Test topic~;
	my $template_quote       = qq~$menusep$img{'replyquote'}~;
	...
	my $message           = qq~$templ_txt{'65'}<br /><a href="#">$templ_txt{'66'}</a>~;
	...
	my $template_move     = qq~$menusep$img{'admin_move'}~;
	my $template_remove   = qq~$menusep$img{'admin_rem'}~;
	my $template_splice   = qq~$menusep$img{'admin_splice'}~;
