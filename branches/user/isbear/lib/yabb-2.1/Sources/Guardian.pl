...
$guardianplver = 'YaBB 2.1 $Revision: 1.3 $';
...
$abuse_time = &timeformat($date);
$abuse_time =~ s/\<sup\>\*\<\/sup\>/\*/g;
...
sub guard {
	...
	if ($ENV{REQUEST_METHOD} eq 'POST') {
		...
		chomp $testform;
		if ($testform ne $decryptformsession || $fsession eq "") {
			&fatal_error("$maintxt{'801'} $user_ip");
		}
	}
	...
	if ($disallow_proxy_on && !$whitelisted && !$iamadmin && ($proxy1 ne "empty" || $proxy2 ne "empty" || $proxy3 ne "empty")) {
		if ($disallow_proxy_notify) {
			...
			$not_body .= qq~$guardian_txt{'abuse_user'}: $username -> ($realname)\n~;
		...
		&fatal_error("$guardian_txt{'proxy_reason'}");
	}
	...
	# Check for Referer
	if ($referer_on) {
		@refererlist = split(/\|/, $banned_referers);
		...
		foreach (@refererlist) {
			...
			if ($streferer =~ m/$_/ && $_ ne "") {
				...
				&fatal_error("$guardian_txt{'referer_reason'}");
			}
		}
	}
	...
	# Check for Harvester
	if ($harvester_on) {
		@harvesterlist = split(/\|/, $banned_harvesters);
		...
		foreach (@harvesterlist) {
			...
			if ($agent =~ m/$_/ && $_ ne "") {
				...
				&fatal_error("$guardian_txt{'harvester_reason'}");
			}
		}
	}
	...
	# Check for Request
	if ($request_on) {
		@requestlist = split(/\|/, $banned_requests);
		...
		foreach (@requestlist) {
			...
			if ($method =~ m/$_/ && $_ ne "") {
				...
				&fatal_error("$guardian_txt{'request_reason'}");
			}
		}
	}
	...
	# Check for Strings
	if ($string_on) {
		@stringlist = split(/\|/, $banned_strings);
		foreach (@stringlist) {
			chomp $_;
			if ($querystring =~ m/$_/ && $_ ne "") {
				if ($string_notify) {
				...
				&fatal_error("$guardian_txt{'string_reason'} ($_)");
			}
		}
	}
	...
	if ($union_on) {
		if ($querystring =~ m/%20union%20/ || $querystring =~ m/\*\/union\/\*/) {
			...
			&fatal_error($guardian_txt{'union_reason'});
		}
	}
	...
	if ($clike_on) {
		if ($querystring =~ m/\/\*/) {
			...
			&fatal_error($guardian_txt{'clike_reason'});
		}
	}
	...
	if ($script_on) {
		while (($key, $secvalue) = each(%INFO)) {
			...
			if (($secvalue =~ m/<[^>]script*\"?[^>]*>/) || ($secvalue =~ m/<[^>]*object*\"?[^>]*>/) || ($secvalue =~ m/<[^>]*iframe*\"?[^>]*>/) || ($secvalue =~ m/<[^>]*applet*\"?[^>]*>/) || ($secvalue =~ m/<[^>]*meta*\"?[^>]*>/) || ($secvalue =~ m/<[^>]*style*\"?[^>]*>/) || ($secvalue =~ m/<[^>]*form*\"?[^>]*>/) || ($secvalue =~ m/\([^>]*\"?[^)]*\)/) || ($secvalue =~ m/\"/)) {
				...
				&fatal_error($guardian_txt{'script_reason'});
			}
		}
